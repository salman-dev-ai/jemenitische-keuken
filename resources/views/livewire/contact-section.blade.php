...
<section id="contact"
    class="py-20 bg-[#FAF6F0] text-[#2C1810] relative overflow-hidden border-b border-[#E8DFD3] font-['Tajawal',sans-serif]"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {{-- نموذج التواصل --}}
            <div
                class="lg:col-span-6 bg-white p-6 sm:p-8 rounded-3xl border border-[#E8DFD3] shadow-lg text-right space-y-6">
                <h3 class="text-lg font-extrabold text-[#2C0D0A]"><span>{{ __('messages.contact.form.title') }}</span>
                </h3>
                ...
                <form wire:submit.prevent="submitMessage" class="space-y-4">
                    <div>
                        <label
                            class="block text-xs font-bold text-stone-700 mb-1.5"<span>{{ __('messages.contact.form.name') }}</span></label>
                        <input type="text" wire:model.defer="name" required
                            class="w-full px-4 py-3 rounded-xl bg-stone-50 border border-stone-200 text-xs">
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        <div>
                            <label
                                class="block text-xs font-bold text-stone-700 mb-1.5"<span>{{ __('messages.contact.form.email') }}</span></label>
                            <input type="email" wire:model.defer="email" required
                                class="w-full px-4 py-3 rounded-xl bg-stone-50 border border-stone-200 text-xs">
                        </div>
                        <div>
                            <label
                                class="block text-xs font-bold text-stone-700 mb-1.5"><span>{{ __('messages.contact.form.phone') }}</span></label>
                    ...
                    <div>
                        <label
                            class="block text-xs font-bold text-stone-700 mb-1.5"><span>{{ __('messages.contact.form.message') }}</span></label>
                        <textarea rows="4" wire:model.defer="message" required
            ...
            <div class="lg:col-span-6 space-y-6 text-right">
                ...
                <div class="bg-white rounded-3xl overflow-hidden border border-[#E8DFD3] shadow-md">
                    <iframe
                        src="https://maps.google.com/maps?q=Damrak%2045%20Amsterdam&t=&z=15&ie=UTF8&iwloc=&output=embed"
